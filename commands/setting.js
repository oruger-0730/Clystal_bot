...
const earthquakePath = './json/earthquake.json';
...
module.exports = {
  ...
  const leavepath = "./json/leavemessage.json";
  const channel = interaction.options.getChannel("channel");
  const message = interaction.options.getString("message");
  ...
  if (fs.existsSync(leavepath)) {
    try {
      config = JSON.parse(fs.readFileSync(leavepath, "utf8"));
    } catch (err) {
  ...
  fs.writeFileSync(leavepath, JSON.stringify(config, null, 2), "utf8");
